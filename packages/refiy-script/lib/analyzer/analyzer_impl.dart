...
class RSAnalyzerImpl implements AbstractASTVisitor<void> {
  final RSLexicon _lexicon;

  final typeChecker = RSTypeChecker();

  /// Errors of a single file
  late List<RSAnalysisError> errors = [];

  RSAnalyzerImpl({required RSLexicon lexicon}) : _lexicon = lexicon;
  ...
  void visitIdentifierExpr(IdentifierExpr node) {
    ...
    } on RSError catch (err) {
      errors.add(RSAnalysisError.fromError(
        err,
